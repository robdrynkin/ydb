...
Y_UNIT_TEST_SUITE(ErasureBlock82) {
    ...
    Y_UNIT_TEST(OldSpeciesGeometryIsUnchanged) {
        // Frozen geometry of the existing species still supported by the registry.
        constexpr std::array<std::array<ui32, 3>, 6> geometry{{
            {TErasureType::ErasureNone, 1, 1},
            {TErasureType::Erasure4Plus2Block, 32, 4},
            {TErasureType::ErasureMirror3dc, 1, 1},
            {TErasureType::Erasure4Plus3Block, 32, 4},
            {TErasureType::Erasure3Plus3Block, 16, 3},
            {TErasureType::ErasureMirror3of4, 1, 1},
        }};
        for (const auto& [species, column, count] : geometry) {
            const TErasureType type{TErasureType::EErasureSpecies(species)};
            const ui32 block = column * count;
            UNIT_ASSERT_VALUES_EQUAL_C(type.ColumnSize(), column, species);
            UNIT_ASSERT_VALUES_EQUAL_C(type.MinimalBlockSize(), block, species);
            for (ui32 size : {0, 1, 7, 8, 15, 16, 31, 32, 33, 127, 128, 129, 700, 1000}) {
                const ui64 user = (size + block - 1) / block * column;
