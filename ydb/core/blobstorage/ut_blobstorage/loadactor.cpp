...
TLoadRunMetrics RunLoadScenario(const TLoadScenarioSettings& settings) {
        ...
        switch (ev->GetTypeRewrite()) {
            ...
            case TEvBlobStorage::EvGet: {
                ...
                if (get->QuerySize) {
                    ...
                    if (tabletIdToGroupIdx.find(id.TabletID()) != tabletIdToGroupIdx.end() && id.Step() != Max<ui32>()) {
                        ++metrics.LoadGets;
                    }
                }
                break;
    ...
    conf << "Instances: " << settings.Instances << "\n";
    conf << "Tablets: {\n";
    ...
    if (!settings.WaitForFinish) {
        ...
        for (ui32 i = 0; i < maxIterations &&
                (metrics.LoadPuts < settings.MinPutRequests || metrics.LoadGets < settings.MinGetRequests); ++i) {
            bool iteration = true;
            env.Runtime->Sim([&] {
...
Y_UNIT_TEST_SUITE(LoadActorSharedDispatching) {
    Y_UNIT_TEST(StorageLoadMultipleInstancesIncreaseInflight) {
        TLoadScenarioSettings singleInstance{
        ...
        auto single = RunLoadScenario(singleInstance);
        UNIT_ASSERT_C(single.MaxInFlightTotal >= 2,
            TStringBuilder() << "Expected single-instance in-flight >= 2, got " << single.MaxInFlightTotal);

        TLoadScenarioSettings twoInstances = singleInstance;
        twoInstances.Instances = 2;

        auto multi = RunLoadScenario(twoInstances);
        UNIT_ASSERT_C(multi.LoadPuts >= singleInstance.MinPutRequests,
            TStringBuilder() << "Expected puts in multi-instance mode, got " << multi.LoadPuts);
        UNIT_ASSERT_C(multi.MaxInFlightTotal > single.MaxInFlightTotal,
            TStringBuilder() << "Expected larger in-flight in 2-instance mode, single# "
                             << single.MaxInFlightTotal << " multi# " << multi.MaxInFlightTotal);
    }
